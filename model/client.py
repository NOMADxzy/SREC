...
class Client:
    ...
    def run(self, max_steps):
        ...
        for r in range(max_steps):
            ...
            abr_actions, abr_action, abr_strength = self.abr_trainer.get_exploration_action(state[:ABR_STATE_DIM])
            place_actions, place_action, place_strength = self.place_trainer.get_exploration_action(
                np.asarray([state[0], state[-2], state[-1]]))
